(() => {
  ...
  function initTheme() {
    let p = "dark";
    try {
      p = localStorage.getItem("litewebui_theme") || "dark";
    } catch { /* ignore */ }
    applyTheme(p);
    try {
      ...
      const onChange = () => {
        try {
          if ((localStorage.getItem("litewebui_theme") || "dark") === "system") applyTheme("system");
        } catch { /* ignore */ }
      };
  ...
  async function openSettings() {
    ...
    try {
      syncThemeCards(localStorage.getItem("litewebui_theme") || "dark");
    } catch {
      syncThemeCards("dark");
    }
  ...
  async function newChat() {
    const r = await api("/api/conversations", {
      method: "POST",
      body: JSON.stringify({ title: "New chat", model: modelSel ? modelSel.value : "" }),
    });
  ...
  function appendMsg(role, content, rawStreaming, atts, msgId) {
    ...
    if (role === "user") {
      ...
      av.className = "avatar";
      av.setAttribute("aria-label", "litewebui");
      av.innerHTML = '<img class="avatar-logo" src="/favicon.svg" alt="" width="20" height="20" />';
      const body = document.createElement("div");
      body.className = "body";
      const roleEl = document.createElement("div");
      roleEl.className = "role";
      roleEl.textContent = displayModelName(modelSel && modelSel.value) || "litewebui";
      bubble = document.createElement("div");
      bubble.className = "bubble";
      if (rawStreaming) {
  ...
  function displayModelName(m) {
    const raw = (m || "").trim();
    if (!raw) return "litewebui";
    if (/^komboku$/i.test(raw)) return "litewebui";
    return raw;
  }

  function syncEmptyTitle() {
    const t = $("#empty-title");
    if (t) t.textContent = "litewebui";
    const av = $("#empty-avatar");
    if (av && !av.querySelector("img")) {
      av.innerHTML = '<img class="ds-logo-img" src="/favicon.svg" alt="" />';
    }
  }
